...
class session (pgraph.graph):
    ...
    def fuzz (self, this_node=None, path=[]):
        ...
        elif self.fuzz_mode=="manual":

            target = self.targets[0]
            done_with_fuzz_node = False
            read_num=0
            with open(self.case_filename) as tf:
                temp_num=sum(1 for x in tf)
                case_num=(temp_num-1)/2
            if case_num==-1:
                case_num=0
            print case_num
            cf=open(self.case_filename,"r")
            # loop through all case.
            while not done_with_fuzz_node:
                # if we need to pause, do so.
                self.pause()
                if read_num==case_num:
                    self.logger.error("all cases exhausted......")
                    done_with_fuzz_node = True
                    continue

                # exception error handling routine, print log message.
                def error_handler (e, msg, target, sock=None):
                    if sock:
                        sock.close()                    
                    msg += "\nException caught: %s" % repr(e)
                ...
                # if we don't need to skip the current test case.
                if read_num <= case_num:
                    self.logger.error("fuzzing %d of %d" % (read_num, case_num))
                    ...
                    # now send the current node we are fuzzing.
                    try:
                        self.transmit_change(sock,read_num,case_num,cf)
                    except Exception, e:
    ...
    ####################################################################################################################
    def transmit_change (self,sock,read_num,case_num,temp_filename):
        data = None
        try:
            if self.proto == socket.SOCK_STREAM:
                data=cPickle.load(temp_filename)
                sock.send(data)
                #print data
            else:
                data=cPickle.load(temp_filename)
...
#
    ...
    ####################################################################################################################
    def transmit (self, node, edge, target):
        '''
        ...
        if not data:
            data = node.render()
        cf=open(self.case_filename,"a+")
        cPickle.dump(data,cf)
        cf.close()
