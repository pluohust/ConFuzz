...
VOID ImageLoad(IMG img, VOID *v)
{
    ...
    {
        RTN_Open(rtnmalloc);
        RTN_InsertCall(rtnmalloc, IPOINT_BEFORE, AFUNPTR(BeforeMalloc), IARG_FUNCARG_ENTRYPOINT_VALUE, 0, IARG_THREAD_ID, IARG_END);
        RTN_InsertCall(rtnmalloc, IPOINT_AFTER, AFUNPTR(AfterMalloc), IARG_FUNCRET_EXITPOINT_VALUE, IARG_THREAD_ID, IARG_END);
        RTN_Close(rtnmalloc);
    }
